...
.. raw:: html
    ...
    <script type="text/javascript">
        ...
        var texts = [
            ...
            "Step 2: After loading the dataset, the user can then manually select columns (quasi-identifiers) by clicking on the field in the Select Column field (highlighting them in blue).<br>" + 
            "The field Unique Value indicates how many unique values can be found in the column. The Type field shows whether or not a column is continuous or categorical, " +
            ...
            "<p style='margin-top: 16px;'> The user will be asked to fill out both the specificed max size of attribute to consider this can be a minimum of 1 field and max of max number of fields available.<br>" +
            " The Data Intrusion Simulation (DIS) metrics this has to be between 0 to 1 default is set to: 0.2. The higher the DIS the more background knowledge we assume an adversary have.",
            ...
            " Importantly the user has the option to specify if there is any missing values in form of (NaN) values or other entires indicating missing values. In this case the computation of CIG will automatically set these to 0. Ensuring that they do not count towards a privacy risk.",

            "Sep 12: This will then prompt the CIG value for all individual cell values. To get the privacy risk for the row, the RIG is computed and displayed in the out most right field." +
            " For the dataset we see that row with index 169, 167, 37, 52 & 159 are the top 5 rows with highest risk." +
            " Additionally we see that PIF is 21.38, higher than the recommended guidelines seen below." +

            

            "<p style='margin-top: 16px;'>  Example thresholds for different Data Safe levels:" +
            ...
            "<p style='margin-top: 16px;'> The user can based on this information choose to anynomise the data further by use of the methods from the peview page or they can manually outside the program discard rows with a user specficed theshold", 
            ...
            "<p style='margin-top: 16px;'> For example here we see that Age has a very high contribution to the risk factor. Whereas Sex and Marital-Status contribute less.",

            "Step 14: After having performed some of the anonymising techniques from the preview page we can see a big improvement to our over Privacy information Factor." +
            "<p style='margin-top: 16px;'> The user is adviced to keep track of the trade-off between utility and privacy hence there is not a one solution fit all, in regards to which anonymisation techniques to use but rather a suggestion to what could be done, to improve the avaliable risk privacy measures from the metaprivBIDS platform." ,
