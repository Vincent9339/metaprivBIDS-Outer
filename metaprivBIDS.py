...
class FileAnalyzer(QMainWindow):
    ...
    def generate_heatmap(self):
        if hasattr(self, 'cigs_df') and not self.cigs_df.empty:
            ...
            self.close_heatmap_button.show()  
        else:
    ...
    def compute_suda(self):
        ...
        suda_result['MAD'] = mad_formatted
        
        # Debugging output
        print("SUDA Result Head:\n", suda_result.head())
        
       
        self.show_results_dialog(suda_result, result_type="suda")
    ...
    def show_results_dialog(self, results_df, result_type="combined"):
        ...
        if ok and column_name:
            ...
            if ok and precision:
                try:
                    ...
                    if column_name in self.data.columns:
                        self.original_columns.setdefault(column_name, self.data[column_name].copy())  
